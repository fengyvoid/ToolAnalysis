  ...
  for(int bi: NReadBoards)
    {
        ...
        if(FindT0VerbosityLevel>0) cout<<"In LAPPDTimeAlignment, T0 channel:"<< T0channelNo<<" , InputWavlabel: "<<InputWavLabel<<" , LAPPDdata.size()="<<lappddata.size()<<endl;
    ...
    for (itr = lappddata.begin(); itr != lappddata.end(); ++itr)
    {
        ...
        if(FindT0VerbosityLevel>2)cout <<"Looping with channelno: "<<channelno<<" , bi: "<<bi<<" , countnumber: "<<countnumber<<endl;

        //loop over all Waveforms
        for(int i=0; i<Vwavs.size(); i++)
        {
          if(FindT0VerbosityLevel>2 && i ==0) cout<<"Looping waveform at"<<i<<endl;
    ...
    if(FindT0VerbosityLevel>0) cout<<"End of LAPPDTimeAlignment..."<<endl;
